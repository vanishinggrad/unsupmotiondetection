...
class ParseDataFile:
    ...
    def parse(self, filename, has_payload):
        ...
        count = 0
        offset = 0
        valid_cnt, less_cnt, zero_cnt = 0, 0, 0
        rssi = []
        while offset < file_size-self.fmt_sz:
            ...
            if csi_len > 0:
                ...
                if min_abs >= 1:
                    ...
                    rssi.append(current_format.rssi)
        ...
        rssi = np.array(rssi)
        print('valid cnt {} less antenna cnt {} zero entry cnt {} mean rssi {} var rssi {}'.format(valid_cnt, less_cnt, zero_cnt,
            np.mean(rssi), np.var(rssi)))
        return frame_data
